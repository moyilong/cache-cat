...
use crate::raft::types::entry::request::RedisOperation::RedisSUnionStore;
...
pub struct SUnionStoreParams {
    pub key: Bytes,
    pub keys: Vec<Bytes>,
}

impl Display for SUnionStoreParams {
...
impl RaftCommand for SUnionStoreCommand {
    fn raft_request(&self, items: &[Value]) -> Result<Operation, ProtocolError> {
        ...
        Ok(Operation::Redis(RedisSUnionStore(params)))
    }
}
...
impl Command for SUnionStoreCommand {
    async fn execute(
        ...
        server
            .app
            .write(Operation::Redis(RedisSUnionStore(params)), client.db_number)
            .await
    }
}
